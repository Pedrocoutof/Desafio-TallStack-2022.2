<div class="px-2">
    ...
    <form wire:submit.prevent="cadastrarTime" class="flex items-center">
        ...
        <div class="sm:grid sm:grid-cols-2 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5 w-full max-w-lg">
            ...
            <div>
                ...
                <input type="text" wire:model="pais_origem" id="age" autocomplete="age"
                       class="w-full flex-1 flex-1 block w-full focus:ring-indigo-500 focus:border-indigo-500 min-w-0 rounded-md sm:text-sm border-gray-300">
                @error('age') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>

            <div>
                <label for="name" class="text-sm font-medium text-gray-700">Pontuação</label>
                <input type="number" wire:model="pontuacao" id="name" autocomplete="name"
                       class="w-full flex-1 flex-1 block w-full focus:ring-indigo-500 focus:border-indigo-500 min-w-0 rounded-md sm:text-sm border-gray-300">
    ...
    <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
        ...
        @foreach($times as $time)
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="py-3 px-12">
                    <button wire:click="deletaPeloId({{$time->id}})">Deletar</button>
                </td>
            </tr>
        @endforeach
        </tbody>
